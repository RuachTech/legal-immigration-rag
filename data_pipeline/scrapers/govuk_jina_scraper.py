...
class ScrapedChunk:
    ...
    def to_dict(self) -> Dict:
        ...
        return {"metadata": asdict(self.metadata), "text": self.text}
...
"""
    ...
    async def scrape_multiple(
        self, urls: Dict[str, str], max_concurrent: int = 1
    ) -> List[ScrapedChunk]:
        """
        ...
        # Process all URLs
        tasks = [scrape_with_semaphore(part_name, url) for part_name, url in urls.items()]
    ...
    async def scrape_all(self) -> List[ScrapedChunk]:
        ...
        urls = {self.extract_part_name_from_url(url): url for title, url in links.items()}
    ...
    def parse_markdown_content(
        self, markdown_content: str, url: str, part_name: str
    ) -> List[ScrapedChunk]:
        """
        ...
        for section in sections:
            chunk = self._create_chunk_from_section(section=section, part_name=part_name, url=url)
            if chunk:
                chunks.append(chunk)
    ...
    def _create_chunk_from_section(
        self, section: Dict[str, Any], part_name: str, url: str
...
async def scrape_immigration_rules(
    api_key: str, save_results: bool = True, data_dir: Optional[Path] = None
) -> List[ScrapedChunk]:
    """
    ...
    async with GovUKJinaScraper(
        api_key=api_key, save_results=save_results, data_dir=data_dir
    ) as scraper:
        return await scraper.scrape_all()


async def scrape_single_page(
    api_key: str, url: str, save_results: bool = True, data_dir: Optional[Path] = None
) -> List[ScrapedChunk]:
    """
    Scrape a single GOV.UK Immigration Rules page.
...
if __name__ == "__main__":
    ...
    if api_key and len(sys.argv) > 1 and sys.argv[1] == "--scrape":
        ...
        url = (
            sys.argv[2]
            if len(sys.argv) > 2
            else "https://www.gov.uk/guidance/immigration-rules/immigration-rules-appendix-fm-family-members"
        )
