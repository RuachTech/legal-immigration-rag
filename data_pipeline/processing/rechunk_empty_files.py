...
async def rechunk_file(chunk_file: Path, raw_file: Path, model: str, url: str) -> None:
    ...
    chunk_file.write_text(json.dumps(output_data, ensure_ascii=False, indent=2), encoding="utf-8")

    logger.info(f"✓ Re-chunked {chunk_file.name} ({len(enhanced_chunks)} chunks)")


async def main() -> None:
    parser = argparse.ArgumentParser(description="Re-chunk empty chunk files from raw markdown")
    parser.add_argument(
        "--chunks-dir",
        type=Path,
