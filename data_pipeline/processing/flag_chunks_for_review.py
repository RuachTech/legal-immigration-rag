...
def should_flag_for_review(chunk: Dict, part_name: str) -> tuple[bool, List[str]]:
    ...
    problematic_files = ["appendix-statelessness", "appendix-eu-family-permit"]
...
def main():
    ...
    for chunk_file in chunk_files:
        ...
        if stats["flagged_chunks"] > 0:
            pct = (
                (stats["flagged_chunks"] / stats["total_chunks"] * 100)
                if stats["total_chunks"] > 0
                else 0
            )
            print(
                f"  {stats['file']}: {stats['flagged_chunks']}/{stats['total_chunks']} chunks flagged ({pct:.1f}%)"
            )
            for reason, count in stats["reasons"].items():
